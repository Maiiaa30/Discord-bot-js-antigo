...
exports.run = async (client, message, args) => {
    if(!message.member.roles.some(r => ["Master", "👑Fundador👑", "REI desta merda"].includes(r.name))) return message.reply("Desculpa, mas não podes fazer isso.");
    ...
    if(!rMember.roles.has(gRole.id)) return message.reply("Esse usuário não tem essa role.");
    await(rMember.removeRole(gRole.id));

    try{
        await message.channel.send(`Parabêns, removeste a role de ${gRole.name} a <@${rMember.id}>`)
    } catch(e) {
